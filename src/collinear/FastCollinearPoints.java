...
//            Point[] pointsInSlopeOrder = Arrays.copyOfRange(points1, i + 1, numberOfPoints);
            ...
            for (int j = 1; j < pointsInSlopeOrder.length; j++) { // find points with the same slope
                ...
                if (Double.compare(prevSlope,slope) == 0) {
                    ...
                    if (collinearPoints.size() >= minNumOfPointsOnLine - 1) {
                        collinearPoints.add(p);
                        addLineSegment(lineSegmentList, collinearPoints, p);
                    }
        ...
        segments = new LineSegment[lineSegmentList.size()];
        lineSegmentList.toArray(segments);
        lineSegmentList.clear();
    }

    private void addLineSegment(List<LineSegment> lineSegments, List<Point> collinearPoints, Point currPoint) {
        if (Collections.min(collinearPoints).compareTo(currPoint) >= 0)
            lineSegments.add(new LineSegment(Collections.min(collinearPoints), Collections.max(collinearPoints)));
    }
